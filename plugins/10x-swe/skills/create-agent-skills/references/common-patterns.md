...
<flexible_guidance>
Use when the assistant should adapt the format based on context:
...
<principle>
Choose one term and use it throughout the skill. Inconsistent terminology confuses the assistant and reduces execution quality.
</principle>
...
the assistant must now interpret: Are "API routes" and "URLs" the same? Are "fields", "boxes", "elements", and "controls" the same?
</bad_example>
...
the assistant must now research and compare all options before starting. This wastes tokens and time.
</bad_example>
...
**Why it matters**: Vague descriptions prevent the assistant from discovering and using the skill appropriately.
</pitfall>
...
**Why it matters**: the assistant may only partially read deeply nested files. Keep references one level deep from SKILL.md.
</pitfall>
...
<description>
Keep SKILL.md concise by linking to detailed reference files. the assistant loads reference files only when needed.
</description>
...
- the assistant only reads relevant reference files
...
- the assistant can fix issues without guessing
...
<checklist_pattern>
<description>
For complex multi-step workflows, provide a checklist the assistant can copy and track progress.
</description>
