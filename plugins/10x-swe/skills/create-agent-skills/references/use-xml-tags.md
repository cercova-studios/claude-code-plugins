...
**Purpose**: Immediate, actionable guidance. Gets the assistant started quickly without reading advanced sections.
...
<tag name="context">
**When to use**: Background or situational information that the assistant needs before starting.
...
**When to use**: Common mistakes that the assistant should avoid.
...
```xml
<testing>
Test with all target models (small, balanced, and large reasoning models):

1. Run skill on representative tasks
2. Observe where the assistant struggles or succeeds
...
## Success criteria
```
Total: ~20 tokens, no semantic meaning to the assistant
...
<parsing_accuracy>
XML provides unambiguous boundaries and semantic meaning. the assistant can reliably:
- Identify section boundaries
- Understand content purpose
- Skip irrelevant sections
- Parse programmatically

Markdown headings are just visual formatting. the assistant must infer meaning from heading text.
</parsing_accuracy>
...
Reduce parsing errors. the assistant knows exactly where sections begin and end.
</benefit>
