...
- No XML tags
- Avoid vendor-specific names unless intentionally provider-scoped
...
- ❌ `helper` (vague)
- ❌ `vendor-helper` (too provider-specific)
</name_field>
...
- Provider-specific names (unless intentional): `openai-tools`, `anthropic-helper`
...
When user asks about revenue, the assistant reads only finance.md. Other files stay on filesystem consuming zero tokens.
</pattern>
...
<critical_rules>
**Keep references one level deep**: All reference files should link directly from SKILL.md. Avoid nested references (SKILL.md → advanced.md → details.md) as the assistant may only partially read deeply nested files.
...
the assistant navigates your skill directory using bash commands:
...
Keep references one level deep from SKILL.md. the assistant may only partially read nested files (SKILL.md → advanced.md → details.md).
</pitfall>
