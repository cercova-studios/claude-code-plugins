...
When the assistant executes this, the full command with expanded `$API_KEY` appears in the conversation.
</the_problem>

<the_solution>
Use `${AGENT_ROOT}/scripts/secure-api.sh` - a wrapper that loads credentials internally.

<for_supported_services>
```bash
# ✅ GOOD - No credentials visible
${AGENT_ROOT}/scripts/secure-api.sh <service> <operation> [args]

# Examples:
${AGENT_ROOT}/scripts/secure-api.sh facebook list-campaigns
${AGENT_ROOT}/scripts/secure-api.sh ghl search-contact "email@example.com"
```
</for_supported_services>
...
1. **Add operations to the wrapper** (`${AGENT_ROOT}/scripts/secure-api.sh`):
...
3. **Add credential placeholders to `${AGENT_ROOT}/.env`** using profile naming:

```bash
# Check if entries already exist
grep -q "YOURSERVICE_MAIN_API_KEY=" ${AGENT_ROOT}/.env 2>/dev/null || \
  echo -e "\n# Your Service - Main profile\nYOURSERVICE_MAIN_API_KEY=\nYOURSERVICE_MAIN_ACCOUNT_ID=" >> ${AGENT_ROOT}/.env

echo "Added credential placeholders to ${AGENT_ROOT}/.env - user needs to fill them in"
```
...
1. **Check for saved profile:**
   ```bash
   ${AGENT_ROOT}/scripts/profile-state get yourservice
   ```

2. **If no profile saved, discover available profiles:**
   ```bash
   ${AGENT_ROOT}/scripts/list-profiles yourservice
   ```
...
5. **Save user's selection:**
   ```bash
   ${AGENT_ROOT}/scripts/profile-state set yourservice <selected_profile>
   ```
...
7. **Make API call with profile:**
   ```bash
   ${AGENT_ROOT}/scripts/secure-api.sh yourservice:<profile> list-items
   ```
...
```bash
${AGENT_ROOT}/scripts/secure-api.sh yourservice:<profile> <operation> [args]

# Examples:
${AGENT_ROOT}/scripts/secure-api.sh yourservice:main list-items
${AGENT_ROOT}/scripts/secure-api.sh yourservice:main get-item <ITEM_ID>
```
...
Usage:
```bash
echo '{"name":"value"}' | ${AGENT_ROOT}/scripts/secure-api.sh service create-item
```
</post_with_json_body>
...
<credential_storage>
**Location:** `${AGENT_ROOT}/.env` (global for all skills, accessible from any directory)
...
```bash
set -a
source ${AGENT_ROOT}/.env 2>/dev/null || { echo "Error: ${AGENT_ROOT}/.env not found" >&2; exit 1; }
set +a
```
</credential_storage>
...
3. **Auto-create credential placeholders** - add empty fields to `${AGENT_ROOT}/.env` immediately when creating the skill
4. **Keep credentials in `${AGENT_ROOT}/.env`** - one central location, works everywhere
...
```bash
# Check .env exists
ls -la ${AGENT_ROOT}/.env

# Check specific variables (without showing values)
grep -q "YOUR_API_KEY=" ${AGENT_ROOT}/.env && echo "API key configured" || echo "API key missing"
```
</testing>
